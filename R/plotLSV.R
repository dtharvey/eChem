...
#' full data files created using \code{lsvSim}, or reduced data
#' \code{sampleLSV}.
#'
...
#' @param line_types A vector of line types for the individual linear sweep voltammograms; defaults to a set of different line types, but can be adjusted by supplying a vector with desired values.
#' @param line_colors A vector of colors for the individual linear sweep voltammograms, whether displayed as lines or as points; defaults to a common color, but can be adjusted by supplying a vector with desired values.
...
plotLSV = function(filenames = list(file1 = NULL, file2 = NULL),
                        legend_text = NULL,
...
# verify that files are for a CV or an LSV simluation

  for (i in 1:numfiles){
    if(filenames[[1]]$expt != "LSV"){
      stop("All files must be for a linear sweep voltammetry simulation.")
    }
  }
