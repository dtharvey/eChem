...
#' full data files created using \code{ccSim} or reduced data
#' files created using \code{sampleCoulgram}.
#'
#' @param filenames A list giving the names of 1--5 files that contain the results of a simulated chronocoulometry experiment.
#' @param scale Setting to value less than 1 adjusts the \emph{y}-axis limits so that the limits are not defined by the current spike.
#' @param legend_text Optional vector that contains text to include in a legend. Default is NULL, which supresses the legend.
#' @param legend_position legend_position One of \code{topleft}, \code{topright}, \code{bottomleft}, or \code{bottomright}; defaults to \code{topleft}.
...
#'
#' @return Returns a plot that shows time on the \emph{x}-axis and charge on the \emph{y}-axis.
#'
...
plotCC = function(filenames = list(file1 = NULL, file2 = NULL),
                        scale = 1,
                        legend_text = NULL,
...
# determine the number of chronocoulograms and verify that the
...
# verify that files are for a CC simluation

  for (i in 1:numfiles){
    if(filenames[[i]]$expt != "CC"){
      stop("All files must be for a chronocoulometry simulation.")
    }
  }
...
# y-axis by examining the data for all chronocoulograms
